...
Backup and recovery solutions have certain limitations. **Redo Rescue** relies on [sfdisk](https://manpages.debian.org/stretch/util-linux/sfdisk.8.en.html) to backup and restore partition tables, and [partclone](https://manpages.debian.org/stretch/partclone/partclone.8.en.html) to create and restore backups of the data on each partition. Redo Rescue is designed to restore a backup image to the same hardware it was taken from. A quick search for "cloned drive won't boot" should provide ample explanation as to why: Even if the target system is nearly identical, Windows can fail to boot for reasons that have nothing to do with the backup solution used.
