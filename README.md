...
Backup and recovery solutions have certain limitations. **Redo Rescue** relies on [sfdisk](https://manpages.debian.org/stretch/util-linux/sfdisk.8.en.html) to backup and restore partition tables, and [partclone](https://manpages.debian.org/stretch/partclone/partclone.8.en.html) to create and restore backups of the data on each partition. Redo Rescue is designed to restore a backup image to the _same exact hardware_ it was taken from. A quick search for "cloned drive won't boot" should provide ample explanation as to why: Even if the target system is nearly identical, Windows can fail to boot for reasons that have nothing to do with the backup solution used.
...
Given the above, an entire backup can be restored successfully, yet the operating system could fail to boot if the system configuration has changed in certain ways. In these situations all files and data are safely recovered (and can be copied to another system), but the system will not boot. _This is not a limitation of the backup solution, but the result of changes to the configuration._ A quick glance at forums for the leading commercial recovery solutions will quickly illustrate the difficulty these changes present, particularly with Microsoft Windows. **For the foregoing reasons, we strongly recommend creating a new backup image after significant changes are made to your system.**
