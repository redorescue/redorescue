...
Backup and recovery solutions have certain limitations. **Redo Rescue** relies on [sfdisk](https://manpages.debian.org/stretch/util-linux/sfdisk.8.en.html) to backup and restore partition tables, and [partclone](https://manpages.debian.org/stretch/partclone/partclone.8.en.html) to create and restore backups of the data on each partition. Redo Rescue is designed to restore a backup image to the _same exact hardware_ it was taken from. A quick search for "cloned drive won't boot" should provide ample explanation as to why: Even when the target system is _identical_, Windows may refuse to boot for reasons that have absolutely nothing to do with the backup solution used.
