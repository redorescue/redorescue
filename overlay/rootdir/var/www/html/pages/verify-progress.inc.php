...
<h1>Verify</h1>
<h3>Verifying backup image</h3>
<p>Checking the integrity of the selected backup image.</p>
...
function cancel() {
	bootbox.confirm({ 
		message: '<h3>Are you sure?</h3><p>Canceling will abort and the image verification will not be completed!</p>',
		buttons: {
			confirm: {
				label: 'Yes, I\'m sure!',
		...
		.done(function(data) {
			...
			if (r['status']) {
				...
				if (r['done'] != null) {
					...
					$('#again').show();
					$('#exit').show();
					bootbox.alert("<h3><i class='fas fa-check-circle text-success'></i> Verification complete</h3><p>All done! "+r['done']+"</p>");
				}
			} else {
